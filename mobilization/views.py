...
def bank_deposit_summary(request, date):
    mobilization = request.user.mobilization
    bank_deposits = BankDeposit.objects.filter(mobilization=mobilization, status='Approved', date_deposited=date).order_by('-date_deposited', '-time_deposited')
    context = {
        'date': date,
        'bank_deposits': bank_deposits,
...
def payment_summary(request, date):
    mobilization = request.user.mobilization
    payments = PaymentRequest.objects.filter(mobilization=mobilization, status='Approved', created_at=date).order_by('-created_at')
    context = {
        'date': date,
        'payments': payments,
